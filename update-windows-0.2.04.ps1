...
$logPath = Join-Path $logDir "update-0.2.04.log"
$releaseBase = "https://github.com/mundusx/releases/releases/download/cli-windows-v0.2.04"
$assets = @(
  @{ Name = "mundusx.exe"; Download = "mundusx-x86_64-pc-windows-msvc.exe"; Hash = "ceb268ba6f582eeae534eb87489b61ae6aff9d562d85f7b68f3292702428e5bc" },
  @{ Name = "opengpu.exe"; Download = "opengpu-x86_64-pc-windows-msvc.exe"; Hash = "7b491dafbd88ae46a38b5367372051f40ba7a42dc288c0b153ab0aa6602c1870" },
...
$backupDir = Join-Path (Split-Path $installDir -Parent) ("before-0.2.04-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
try {
  ...
  if ($installedVersion -ne "mundusx 0.2.04") { throw "Installed version check failed: $installedVersion" }
  $tray = Join-Path $installDir "mundusx-tray.exe"
  if (Test-Path -LiteralPath $tray) { Start-Process -FilePath $tray }
  Write-Host "MundusX 0.2.04 installed. Backup: $backupDir"
} finally {
