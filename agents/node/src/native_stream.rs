...
impl NativeStream {
    pub fn push(&mut self, delta: &Value) -> Result<(), String> {
        if !delta.is_object() {
            return Err("native delta must be an object".into());
        }
        if let Some(content) = delta.get("content").filter(|v| !v.is_null()) {
            self.content
                .push_str(content.as_str().ok_or("content must be a string")?);
        }
        if let Some(calls) = delta.get("tool_calls") {
            for call in calls.as_array().ok_or("tool_calls must be an array")? {
                let index = call["index"]
                    .as_u64()
                    .filter(|v| *v < 128)
                    .ok_or("invalid tool index")? as usize;
    ...
    pub fn message(&self) -> Value {
        ...
        if !self.calls.is_empty() {
            message["tool_calls"] = self.calls.values().cloned().collect();
        }
        message
    }
    ...
    pub fn remainder(&self, final_message: &Value) -> Result<Value, String> {
        let suffix = |full: &str, prefix: &str| {
            full.strip_prefix(prefix)
                .map(str::to_owned)
                .ok_or_else(|| "native stream differs from final message".to_string())
        };
        let content = suffix(
            final_message["content"].as_str().unwrap_or_default(),
            &self.content,
        )?;
        let mut calls = Vec::new();
        let final_calls = final_message["tool_calls"]
            .as_array()
            .cloned()
            .unwrap_or_default();
        if self.calls.keys().any(|i| *i >= final_calls.len()) {
            return Err("final message lost streamed tool call".into());
        }
        for (index, full) in final_calls.iter().enumerate() {
            let empty = json!({});
            let previous = self.calls.get(&index).unwrap_or(&empty);
            let id = suffix(
                full["id"].as_str().unwrap_or_default(),
                previous["id"].as_str().unwrap_or_default(),
            )?;
            let name = suffix(
                full["function"]["name"].as_str().unwrap_or_default(),
                previous["function"]["name"].as_str().unwrap_or_default(),
            )?;
            let args = suffix(
                full["function"]["arguments"].as_str().unwrap_or_default(),
        ...
        let mut delta = json!({});
        if !content.is_empty() {
            delta["content"] = content.into();
        }
        if !calls.is_empty() {
            delta["tool_calls"] = calls.into();
        }
        Ok(delta)
    }
}
...
mod tests {
    ...
    fn fragments_reconcile_without_duplicate_arguments() {
        ...
        assert_eq!(
            remainder["tool_calls"][0]["function"]["arguments"],
            "hello\"}"
        );
    ...
    fn interleaved_calls_and_content_preserve_order() {
        ...
        stream
            .push(&json!({"tool_calls":[{"index":1,"function":{"arguments":"}"}}]}))
            .unwrap();
        assert_eq!(stream.message()["tool_calls"][0]["id"], "a");
        assert_eq!(
            stream.message()["tool_calls"][1]["function"]["arguments"],
            "{}"
        );
