...
$logPath = Join-Path $logDir "update-0.2.08.log"
$releaseBase = "https://github.com/mundusx/releases/releases/download/cli-windows-v0.2.08"
$assets = @(
  @{ Name = "mundusx.exe"; Download = "mundusx-x86_64-pc-windows-msvc.exe"; Hash = "4a822dfdd4c0df90dd6d55812f85d61973e081538fe79999d6189543c63aad2f" },
  @{ Name = "opengpu.exe"; Download = "opengpu-x86_64-pc-windows-msvc.exe"; Hash = "4e1d83d87777d0d6e6e1f4956ad932b0354eefbebea13e24ef59bdad328bce40" },
...
$backupDir = Join-Path (Split-Path $installDir -Parent) ("before-0.2.08-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
try {
  ...
  if ($installedVersion -ne "mundusx 0.2.08") { throw "Installed version check failed: $installedVersion" }
  $tray = Join-Path $installDir "mundusx-tray.exe"
  if (Test-Path -LiteralPath $tray) { Start-Process -FilePath $tray }
  Write-Host "MundusX 0.2.08 installed. Backup: $backupDir"
} finally {
