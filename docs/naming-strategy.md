...
This repo is building the NovusX product and its local prototype surfaces.

## Current Rule

- `NovusX` is the product and platform name we should use on user-facing surfaces.
...
- the local UI, docs, and public-facing copy should say `NovusX`
- the current command name can remain `opengpu` until we intentionally decide to rename the binary family
- if we ever rename the binary to `novusx`, do it as a deliberate compatibility pass, not as an incidental string swap

## Practical Guidance

- Use `NovusX` in headings, brand labels, page titles, and marketing copy.
...
- If a doc mentions both, prefer wording like:
  - "NovusX is the product; `opengpu` is the current CLI command name."
