...
tray_icon="mundusx.ico"
runtime="llama-runtime-x86_64-pc-windows-msvc-cuda.zip"
for asset in "$binary" "$agent" "$tray" "$tray_icon" "$runtime"; do
...
python - "$tmp_dir/release-manifest.json" "$tray" "$tray_icon" "$runtime" <<'PY'
import json
import sys
...
assert assets[tray_icon_name]["kind"] == "windows-tray-icon"
assert assets[tray_icon_name]["install_as"] == "mundusx.ico"
...
echo "Windows tray, tray icon, and CUDA runtime are present in the signed release manifest"
