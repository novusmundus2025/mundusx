...
assert_contains "docs/install-strategy.md" 'Windows CLI releases should build on GitHub-hosted `windows-latest` runners with the stable Rust MSVC toolchain target `x86_64-pc-windows-msvc`'
assert_contains "docs/install-strategy.md" 'opengpu-x86_64-pc-windows-msvc.exe'
assert_contains "docs/install-strategy.md" 'Release signing secrets should stay shared across platform jobs and be provided only through GitHub Actions secrets'
assert_contains "docs/install-strategy.md" 'add a `windows-latest` matrix row to `.github/workflows/release-cli.yml`'
