                    ...
                    .and_then(|body| {
                        ...
                        .and_then(|response| {
                            if wants_stream {
                                ...
                                collect_openai_response(response, Arc::clone(&worker_stop), progress.clone()).map(ProxyResponse::Json)
                            }
                        })
                    });
                match result {
...
fn collect_openai_response(response: ureq::Response, stop: Arc<AtomicBool>, progress: Option<std::sync::mpsc::Sender<Value>>) -> Result<Value, ProxyError> {
    let status = response.status();
    let mut body = String::new();
    CancellableReader { inner: response.into_reader(), stop, progress: StreamProgress::new(progress) }
        .read_to_string(&mut body).map_err(|error| ProxyError {
        status: 502,
