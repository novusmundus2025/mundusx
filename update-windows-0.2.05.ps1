...
$logPath = Join-Path $logDir "update-0.2.05.log"
$releaseBase = "https://github.com/mundusx/releases/releases/download/cli-windows-v0.2.05"
$assets = @(
...
$backupDir = Join-Path (Split-Path $installDir -Parent) ("before-0.2.05-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
try {
  ...
  if ($installedVersion -ne "mundusx 0.2.05") { throw "Installed version check failed: $installedVersion" }
  $tray = Join-Path $installDir "mundusx-tray.exe"
  if (Test-Path -LiteralPath $tray) { Start-Process -FilePath $tray }
  Write-Host "MundusX 0.2.05 installed. Backup: $backupDir"
} finally {
